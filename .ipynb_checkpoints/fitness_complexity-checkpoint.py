...
def fitness_complexity_mazzolini(bin_rca, max_iteration = 1000, check_stop='distance', min_distance=1e-14, normalization='mean', fit_ic=[], com_ic=[], removelowdegrees=False, verbose=False, redundant=False, add_noise=False, gamma=-1):
    """
    Routine to evaluate the fitness and the complexity from Tacchella et al. (2012), 'A new metrics for countries' Fitness and products Complexity', Sci.Rep.
    ...
    for iterat in range(max_iteration):
        ...
        y[mask_y] = np.power(x[mark_y], gamma)
