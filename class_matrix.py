...
class efc_matrix:
    ...
    def get_fitness(self, aspandas=False, force=False, method=None, max_iteration = 1000, check_stop='distance', min_distance=1e-14, normalization='mean', fit_ic=[], com_ic=[], removelowdegrees=False, verbose=False, consider_dummy=False, delta=1.0): # ok with sparse
        """
    ...
    def get_complexity(self, aspandas=False, force=False, method=None, max_iteration = 1000, check_stop='distance', min_distance=1e-14, normalization='mean', fit_ic=[], com_ic=[], removelowdegrees=False, verbose=False, consider_dummy=False, delta=1.0): # ok with sparse
        """
    ...
    def get_masked_from_indices(self, list_rows=None, list_columns=None):
        ...
        if scipy.sparse.issparse(self.matrix):
            ...
            return efc_matrix(matrix, label_rows=list_rows, label_columns=list_columns)
        else:
            matrix = self.matrix[mask_rows,mask_columns]
            return efc_matrix(matrix, label_rows=list_rows, label_columns=list_columns)
        return None
